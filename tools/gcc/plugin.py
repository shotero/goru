...
class GccPlugin(ToolPlugin):
    name = "gcc"
    binary = "gcc"
    spec_file = "tools/gcc/spec.yaml"
    ...
    def build_args(self, argv: list[str], config: dict[str, Any]) -> list[str]:
        ...
        if not wrapper_args:
            raise ValueError(f"gcc requires a subcommand: {', '.join(subcommands)}")

        subcommand_name = wrapper_args[0]
        subcommand = subcommands.get(subcommand_name)
        if not subcommand:
            raise ValueError(f"unknown gcc subcommand: {subcommand_name}")
    ...
    def _parse_options(self, wrapper_args: list[str], values: dict[str, Any]) -> list[str]:
        ...
        while i < len(wrapper_args):
            ...
            if option:
                ...
                raise ValueError(f"unknown gcc wrapper option: {arg}")
            else:
    ...
    def _apply_positionals(
        ...
        if mode == "none":
            if positionals:
                raise ValueError(f"gcc {subcommand_name} does not accept positional arguments")
            return
        if mode == "sources":
            if positionals:
                values.setdefault("sources", []).extend(positionals)
            return
        raise ValueError(f"gcc subcommand '{subcommand_name}' has unsupported positional mode: {mode}")
    ...
    def _validate(self, subcommand_name: str, subcommand: dict[str, Any], values: dict[str, Any]) -> None:
        if subcommand.get("positional") == "sources" and not values.get("sources"):
            raise ValueError(f"gcc {subcommand_name} requires at least one source")
        for key in subcommand.get("required", []):
            value = values.get(key)
            if value is None or value == [] or value is False:
                raise ValueError(f"gcc {subcommand_name} requires --{str(key).replace('_', '-')}")
    ...
    def translate_native_args(self, argv: list[str]) -> list[str]:
        ...
        return with_passthrough([mode, *sources, *wrapper_options], passthrough)
